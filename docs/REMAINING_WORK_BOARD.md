...
| P1 | Object-db staging evidence | Filesystem JSON is no longer the preferred shared runtime; DB/MinIO needs target-host proof | Docker Compose object-db health passed locally and object-reference verification passes with `checked=211, missing=0` | Final target host has `object-db` health, object-reference verification, and backup evidence recorded |
| P1 | Host deployment evidence | Docker/runbook exists, but the actual host setup still needs proof | Local Docker object-db health evidence captured at `release-artifacts/deployment-check-20260514-docker-object-db.json` | Final target host has recorded health/deployment evidence |
...
- Latest data/config cleanup evidence:
  ...
  `checked_object_refs=211`, `missing=0`.
- Latest data-root backup:
...
- Latest local Docker object-db health artifact:
  ...
  passed against `127.0.0.1:4173` with `storage_backend=object-db`.
...
- Last pushed commits:
  - `61437ca` - object-db Docker runtime cutover
